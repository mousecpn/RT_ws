...
class SingleIntegrator(Dynamic):
    ...
    def init_constants(self):
        self.F3d = torch.eye(6, device=self.device, dtype=torch.float32)
        self.F3d[0:3, 3:] = torch.eye(3, device=self.device, dtype=torch.float32) * self.dt
        self.F_t3d = self.F3d.transpose(-2, -1)

        self.F2d = torch.eye(4, device=self.device, dtype=torch.float32)
        self.F2d[0:2, 2:] = torch.eye(2, device=self.device, dtype=torch.float32) * self.dt
        self.F_t2d = self.F2d.transpose(-2, -1)
    ...
    def integrate_distribution(self, v_dist, x=None):
        ...
        pos_dist_sigma_matrix_t = torch.zeros(sample_batch_dim + [v_dist.components, dim, dim], device=self.device)

        for t in range(ph):
            vel_sigma_matrix_t = vel_dist_sigma_matrix[:, :, t]
            full_sigma_matrix_t = block_diag([pos_dist_sigma_matrix_t, vel_sigma_matrix_t])
            pos_dist_sigma_matrix_t = self.F[..., :dim, :].matmul(full_sigma_matrix_t.matmul(self.F_t)[..., :dim])
