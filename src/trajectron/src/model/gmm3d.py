...
class GMM3D(td.Distribution):
    ...
    def __init__(self, log_pis, mus, log_sigmas, corrs):
        ...
        self.one_minus_rhos = torch.clamp(self.one_minus_rhos, min=1e-5, max=1)
    ...
    def splating(self):
        return GMM2D(self.log_pis, self.mus[...,:2],
                              self.log_sigmas[...,:2], self.corrs[...,0])
    ...
    def get_at_time(self, t):
        return self.__class__(self.log_pis[:, :, t:t+1], self.mus[:, :, t:t+1],
                              self.log_sigmas[:, :, t:t+1], self.corrs[:, :, t:t+1])
    def mode(self):
        """
